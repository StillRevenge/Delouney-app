...
/* Триангуляция */
// 0 — не пересекается, 1 — пересекается, -1 — пересекается в конечной точке s1, 2 — общая точка
function classifyIntersection(s1from, s1to, s2from, s2to) {
	var intersectionType, intersectionTypeDescription;
	// Вычислите 4-й определитель из 2-х отрезков
	...
	// 2 точки s1 на одной стороне s2 или 2 точки s2 на одной стороне s1
	if (determinal1 > 0 || determinal2 > 0) {
		//Не пересекаются
		intersectionType = 0;
	}
	// Оба сегмента охватывают другой
	else if (determinal1 < 0 && determinal2 < 0) {
		//Пересечение. Пересечение является внутренней точкой.
		intersectionType = 1;
	}
	// Сложная ситуация
	else if (determinal1 == 0 && determinal2 == 0) {
		// Определите позиционное соотношение конечных точек, используя координаты x.
		...
		// Если координаты x одинаковы, используйте координаты y
		else {
		...
		// Начало s1 находится слева (вверху) от начала s2.
		if (s1_min < s2_min) {
			// Конец s1 находится слева (вверху) от начала s2.
			if (s1_max < s2_min) {
				// Не пересекаться
				intersectionType = 0;
			}
			else if (s1_max == s2_min) {
				intersectionType = 2; //общая конечная точка
			}
			// End of s1 is on s2
			else if (s1_max <= s2_max) {
				//Пересечение. Пересечение — это сегмент;
				intersectionType = 1;
			}
		}
		// Начало s2 находится слева (вверху) от начала s1.
		else if (s1_min > s2_min) {
			//Конец s2 находится слева (вверху) от начала s1.
			if (s2_max < s1_min) {
				//Не пересекается
				intersectionType = 0;
			}
	...
	// s2 имеет конечную точку на s1
	else if (determinal1 == 0) {
		//Пересечение — пересечение является конечной точкой s2.
		intersectionType = 1;
	}
	// s1 имеет конечную точку на s2
	else if (determinal2 == 0) {
		if (det_s2f_s2t_s1t == 0) {
			//точка запроса находится на границе
			intersectionType = -1;
		}
		else {
			//Пересечение — пересечение является конечной точкой s1.
			intersectionType = 1;
		}
	}
...
// 1-внутри, -1-не внутри, 0-граница, 2-вершина
function insideTriangle(p, triangle) {
	...
	var other2;
	// Точка находится по одну сторону от трех линий
	if ((det1 > 0 && det2 > 0 && det3 > 0) || (det1 < 0 && det2 < 0 && det3 < 0)) {
		return 1;//Inside
	}
	// Точка находится на прямой или ее продолжении
		...
		// Точка находится на одной линии
		if (other2 > 0) {
			return 0;//Граница
		}
	...
	// точка за пределами треугольника
	else {
		return -1;//снаружи
	}
}
...
/*
findBigTriangle(points): Создает большой треугольник, содержащий все точки в качестве своих вершин.

Инициализирует вспомогательные структуры данных и переменные.

Итерируется по точкам (за исключением последних трех) и обрабатывает их:

Выполняет обработку текущей точки в зависимости от ее положения относительно вспомогательных ребер.
Производит операции флипа Делоне для улучшения триангуляции.
Завершает обработку первой точки (0).

Формирует список треугольников на основе данных о гранях и ребрах.

Возвращает список треугольников как результат работы функции.
*/
function computeTriangulation(points) {
	// Обратите внимание, что это НЕ возвращает триангуляцию, а только ее подмножество
